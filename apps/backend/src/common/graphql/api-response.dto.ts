...
export function PaginatedResponse<TItem>(
  ...
  // <<<< Thay ClassType bằng kiểu constructor cơ bản
  @ObjectType({ isAbstract: true, description: 'Response chung có phân trang' })
  abstract class PaginatedResponseClass {
    @Field(() => [TItemClass], {
      nullable: true,
      description: 'Dữ liệu trả về (danh sách)',
    })
    data?: TItem[];
    ...
    @Field(() => Int, {
      nullable: true,
      description: 'Tổng số mục (cho phân trang)',
    }) // <<<< Int đã được import
    totalCount?: number;

    @Field(() => Boolean, {
      nullable: true,
      description: 'Còn trang tiếp theo không (cho phân trang)',
    })
    hasNextPage?: boolean;
  }
...
export function ApiResponse<TData>(TDataClass: new (...args: any[]) => TData) {
  // <<<< Thay ClassType bằng kiểu constructor cơ bản
  @ObjectType({
    isAbstract: true,
    description: 'Response chung cho một đối tượng đơn lẻ',
  })
