...
export class AssignmentsService {
  ...
  async findOne(id: number): Promise<Assignment> {
    const assignment = await this.repository.findOne({ where: { id }, relations: ['user', 'position', 'unit'] });
    if (!assignment) throw new NotFoundException(`Assignment with ID ${id} not found`);
    return assignment;
  }

  async update(id: number, updateAssignmentInput: UpdateAssignmentInput): Promise<Assignment> {
    const assignment = await this.repository.findOne({ where: { id } });
    if (!assignment) throw new NotFoundException(`Assignment with ID ${id} not found`);
    if (updateAssignmentInput.userId !== undefined) assignment.userId = updateAssignmentInput.userId;
    if (updateAssignmentInput.positionId !== undefined) assignment.positionId = updateAssignmentInput.positionId;
    if (updateAssignmentInput.unitId !== undefined) assignment.unitId = updateAssignmentInput.unitId;
  ...
  async findByUserId(userId: number): Promise<Assignment[]> {
    return this.repository.find({ where: { userId }, relations: ['user', 'position', 'unit'] });
  }
