...
export class AssignmentsResolver {
  ...
  async createAssignments(
    ...
    const assignments = await this.assignmentsService.createMany(
      createAssignmentsInput,
    );
    return {
      metadata: createResponseMetadata(
        HttpStatus.ACCEPTED,
        'Tạo nhiều phân công thành công',
      ),
      data: {
        assignments,
      },
  ...
  ): Promise<GetAssignmentsPaginatedResponse> {
    const data = await this.assignmentsService.findByUserId(userId);
    return {
      metadata: createResponseMetadata(
        HttpStatus.OK,
        'Lấy danh sách phân công theo user thành công',
      ),
      data,
      totalCount: data.length,
      hasNextPage: false,
