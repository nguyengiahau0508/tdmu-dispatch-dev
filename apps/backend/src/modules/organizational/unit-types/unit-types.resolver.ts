...
export class UnitTypesResolver {
  ...
  async createUnitType(
    @Args('createUnitTypeInput') createUnitTypeInput: CreateUnitTypeInput,
  ...
  async findAll(
    @Args('input') input: GetUnitTypesPaginatedInput,
  ): Promise<GetUnitTypesPaginatedResponse> {
  ...
  async updateUnitType(
    @Args('updateUnitTypeInput') updateUnitTypeInput: UpdateUnitTypeInput,
  ): Promise<UpdateUnitTypeResponse> {
    ...
    return {
      ...
      data: await this.unitTypesService.update(
        updateUnitTypeInput.id,
        updateUnitTypeInput,
      ),
    };
  }
  ...
  @Roles(Role.SYSTEM_ADMIN)
  async removeUnitType(
    @Args('id', { type: () => Int }) id: number,
  ): Promise<RemoveUnitTypeResponse> {
