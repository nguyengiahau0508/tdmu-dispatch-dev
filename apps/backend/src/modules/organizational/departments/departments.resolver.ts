...
import { GetDepartmentInput, GetDepartmentOutput } from './dto/get-department';
import {
...
export class DepartmentsResolver {
  ...
  @Query(() => GetDepartmentsPaginatedResponse, {
    name: 'departmentsPaginated',
  })
  async getDepartmentsPaginated(
    @Args('input') input: GetDepartmentsPaginatedInput,
  ): Promise<GetDepartmentsPaginatedResponse> {
  ...
  async getAllDepartmentBySearch(
    @Args({ name: 'search', type: () => String }) search: string,
  ): Promise<GetAllDepartmentResponse> {
    const data =
      await this.departmentsService.findAllDepartmentBySearch(search);
    return {
  ...
  async getDepartment(
    @Args('input') input: GetDepartmentInput,
  ...
  async removeDepartment(
    @Args('input') input: RemoveDepartmentInput,
  ): Promise<RemoveDepartmentResponse> {
